...
fn main() {
    const SIZE: u32 = 640;
    let detect = qbarcode_scanner::DetectNet::new(SIZE);
    let path = std::env::args_os()
        .collect::<Vec<_>>()
        .get(1)
        .map(|v| v.to_string_lossy().to_string())
        .unwrap_or("examples/IMG_3105.jpg".to_string());
    ...
    input
        .copy_from(&ss, (SIZE - ss.width()) / 2, (SIZE - ss.height()) / 2)
        .unwrap();

    let raw = input.to_vec();

    let now = std::time::SystemTime::now();
    let ds = detect.detect_with_pixels(raw.as_slice(), SIZE, 0.3, 0.45);
    // let ds = detect.detect(include_bytes!("IMG_3105.jpg"), 0.3);
    let ds = ds
        .iter()
        .filter(|v| v.rect.w > 50 && v.rect.h > 50)
        .map(|v| v.to_owned())
        .collect::<Vec<_>>();
